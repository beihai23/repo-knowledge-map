...
- 每个节点带 `cite`(file://相对路径#L起-L止),每条边带 `evidence`(哪个工具/哪段代码得出)
...
每次产出都跑这几项,写进 report.md:

1. **链接自检**:所有 `file://路径#行号` 必须真实存在。失效的标红、列入 report。这条最重要——溯源失效,整个知识地图就是空中楼阁。
...
4. **透明度**:report 必含 当前 Tier、各层覆盖范围、链接有效率、推断占比。
